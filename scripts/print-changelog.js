...
const MAINTAINERS = [].map(author => author.toLowerCase());
  ...
  for (let i = 0; i < logs.all.length; i += 1) {
    ...
    for (let j = 0; j < prs.length; j += 1) {
      ...
      const res = await fetch(
        `https://github.com/LagrangeLabs/ii-admin-ui/pull/${pr}`,
      );
  ...
  function printPR(lang, postLang) {
    prList.forEach(entity => {
      ...
      if (pr) {
        ...
        console.log(
          `- ${icon} ${str}[#${pr}](https://github.com/LagrangeLabs/ii-admin-ui/pull/${pr})${authorText}`,
        );
      } else {
        console.log(
          `🆘 Miss Match: ${title} -> https://github.com/LagrangeLabs/ii-admin-ui/commit/${hash}`,
        );
      }
    });
  }
